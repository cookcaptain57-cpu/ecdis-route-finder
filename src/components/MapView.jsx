...
// ✅ FIXED: moved outside component — these are pure functions with no
//    dependency on props/state, so they don't need to be recreated on every render
const depth = (lat, lng) => {
  const ocean = Math.abs(Math.sin(lat * 0.08) * Math.cos(lng * 0.08));
  return 5 + ocean * 8000;
};
...
export default function MapView({
  ...
  //    onMapClick is the only prop used inside init.
  ...
  useEffect(() => {
    ...
    const routeLine = L.polyline(latlngs, { color: "#00B4D8", weight: 3 });
    const corridor  = L.polyline(latlngs, { color: "#1565C0", weight: 12, opacity: 0.15 });

    const markers = waypoints.map((w, i) =>
      L.circleMarker([w.lat, w.lon], {
        radius: 5, color: "#00FFB3", fillOpacity: 1,
      }).bindPopup(`Waypoint ${i + 1}`)
    );

    routeRef.current = L.layerGroup([corridor, routeLine, ...markers]).addTo(map);
  ...
  useEffect(() => {
    ...
    if (!shipRef.current) {
      shipRef.current = L.circleMarker(
        ...
        { radius: 8, color: "#FFB000", fillColor: "#FFB000", fillOpacity: 1 }
      ).addTo(map);
    } else {
      shipRef.current.setLatLng([shipPosition.lat, shipPosition.lon]);
    }
  }, [shipPosition, ready]);
  ...
  useEffect(() => {
    ...
    if (layersRef.current.bathy) layersRef.current.bathy.remove();
    ...
    for (let lat = -70; lat < 80; lat += 5) {
      for (let lng = -180; lng < 180; lng += 5) {
        ...
        L.rectangle([[lat, lng], [lat + 5, lng + 5]], {
          color: encColor(d), weight: 0, fillOpacity: 0.1,
        }).addTo(group);
      }
    }
  ...
  // ───────── LOAD LEAFLET ─────────
  // ✅ FIXED: init added to dependency array (safe now that it's in useCallback)
  useEffect(() => {
