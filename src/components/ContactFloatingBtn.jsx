...
export default function ContactFloatingBtn({ setTab }) {
  ...
  const options = [
    { icon: '✉️', label: 'Contact Us',  sub: 'Get in touch',      tab: 'contact' },
    { icon: '💡', label: 'Write to Us', sub: 'Suggestions & bugs', tab: 'contact' },
    { icon: '❓', label: 'FAQ',          sub: 'Quick answers',     tab: 'faq'     },
    { icon: '🧭', label: 'About',        sub: 'About the app',     tab: 'about'   },
    { icon: '⚖️', label: 'Legal',        sub: 'Terms & disclaimer', tab: 'legal'  },
  ];
    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          minWidth: 210,
          animation: 'floatMenuIn 0.2s ease',
        }}>
          <div style={{
            fontSize: '0.58rem', color: 'var(--text3)',
            textTransform: 'uppercase', letterSpacing: '0.12em',
            padding: '4px 8px 8px', marginBottom: 4,
            borderBottom: '1px solid var(--border)',
          }}>
            NavisphereX Support
          </div>
          {options.map(opt => (
            <button key={opt.tab + opt.label}
              onClick={() => handleNav(opt.tab)}
              style={{
          ...
          <div style={{ marginTop: 6, paddingTop: 6, borderTop: '1px solid var(--border)', padding: '8px 10px 4px' }}>
      ...
      <button
        ...
        style={{
          ...
          background: open
            ? 'linear-gradient(135deg,var(--red),#cc2233)'
