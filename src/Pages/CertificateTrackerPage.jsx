...
// ── Anthropic AI Certificate Extraction ───────────────────────────────────
// ⚠️ Production: move this call to a Firebase Cloud Function to protect your API key.
// Development: add REACT_APP_ANTHROPIC_API_KEY=sk-ant-... to .env.local
...
async function extractCertData(file) {
  if (!ANTHROPIC_KEY) throw new Error('ANTHROPIC_KEY not set in .env.local');
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: `You are a maritime certificate parser. Extract all readable information from this document image.
...
Set isUnlimited to true if the document shows 'no expiry', 'unlimited', 'for life', or similar.
Use null for any field not clearly visible. Return only the JSON.`
          }
        ]
      }]
    })
...
function CertificateTrackerPage({ user, notify }) {
  ...
  // ── NEW: AI Extraction state ──────────────────────────────────────────
  // extractModal: null | { certId, file, reviewData, isRenewal, detected, extractError }
  ...
  // ── NEW: AI Extraction functions ──────────────────────────────────────

  // Entry point when user selects file from per-cert upload button
  const handleFileUpload = async (certId, file, isRenewal = false) => {
    ...
    // Open the review panel immediately (shows loading state)
    ...
    try {
      ...
      // Merge: AI data takes priority over existing, existing fills gaps
      const reviewData = {
      ...
      // Track which fields were auto-detected (show 🤖 badge)
      const detected = {};
      ...
      if (extracted.issuingAuthority) detected.notes   = true;
      if (extracted.notes)           detected.notes    = true;

      setExtractModal({ certId, file, reviewData, isRenewal, detected });
    } catch(e) {
      // Extraction failed — still show the review panel for manual entry
      const existing = certs.find(c => c.id === certId) || {};
      setExtractModal({
        certId, file, isRenewal, detected: {}, extractError: e.message,
        reviewData: {
          name: existing.name || '', certNo: existing.certNo || '',
          issueDate: existing.issueDate || '', expiryDate: existing.expiryDate === 'unlimited' ? '' : (existing.expiryDate || ''),
      ...
      notify('Could not auto-read certificate — please fill in details manually', 'error');
    }
    setExtracting(false);
  };

  // Confirm the extraction review form — save to Firestore + upload to Drive
  const confirmExtraction = async () => {
    ...
    try {
      ...
      if (driveToken) {
        const existing = certs.find(c => c.id === certId);
        // On renewal: delete old Drive file first
      ...
      const updated = certs.map(c => c.id === certId ? { ...c, ...patch } : c);
      await saveCerts(updated);
      setExtractModal(null);
      notify(
        driveToken ? '✅ Certificate updated and file saved to your Drive' : '✅ Certificate updated',
        'success'
      );
  ...
  // Add-form: scan file to pre-fill the Add New Certificate form
  const handleAddFormScan = async (file) => {
    ...
    try {
      ...
      if (extracted.isUnlimited) {
        setUnlimitedExpiry(true); detected.expiryDate = true;
      } else if (extracted.expiryDate) {
        updates.expiryDate = extracted.expiryDate; detected.expiryDate = true;
      }
      if (extracted.notes || extracted.issuingAuthority) {
        updates.notes = [extracted.notes, extracted.issuingAuthority ? `Issued by: ${extracted.issuingAuthority}` : null]
          .filter(Boolean).join(' | ');
        detected.notes = true;
      }
      ...
      notify(`✅ Certificate scanned — ${Object.keys(detected).length} fields auto-filled. Review below.`, 'success');
    } catch(e) {
  ...
  // addCert — additive: also uploads addScanFile to Drive if available
  const addCert = async () => {
    ...
    // NEW: Upload scanned file to Drive at same time as adding cert
    if (addScanFile && driveToken) {
      ...
      } catch(e) { notify('Certificate added but Drive upload failed: ' + e.message, 'error'); }
    }
  ...
  // deleteCert — additive: also removes Drive file if one exists
  ...
  const saveQuick = async (id) => {
    ...
    const updated = certs.map(c =>
      c.id === id ? { ...c, expiryDate: finalExpiry, certNo: quickData.certNo || c.certNo } : c
    );
    await saveCerts(updated);
    setQuickId(null);
  ...
  // aiFields param: optional object of field keys that were AI-detected → shows 🤖 badge
  const renderFormFields = (data, setData, isCN, setIsCN, isCC, setIsCC, isUnlim, setIsUnlim, aiFields = {}) => {
    const aiBadge = (field) => aiFields[field]
      ? <span style={{ marginLeft:5, fontSize:'0.58rem', background:'rgba(0,180,216,0.2)',
          color:'var(--cyan)', borderRadius:10, padding:'1px 5px', fontWeight:700 }}>🤖 AI</span>
      : null;
    ...
    return (
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
        ...
        <div className="ff" style={{ margin:0 }}>
          <label className="fl">Certificate / Document No. {aiBadge('certNo')}</label>
  ...
  // ── NEW: Extraction review form (used in per-cert extraction panel) ───
  const renderExtractionFields = () => {
    if (!extractModal?.reviewData) return null;
    const { reviewData, detected } = extractModal;
    const setField = (field, val) =>
      setExtractModal(m => ({ ...m, reviewData: { ...m.reviewData, [field]: val } }));

    const badge = (field) => detected?.[field]
      ? <span style={{ marginLeft:5, fontSize:'0.58rem', background:'rgba(0,180,216,0.22)',
          color:'var(--cyan)', borderRadius:10, padding:'1px 5px', fontWeight:700 }}>🤖 AI</span>
      : null;
    ...
    return (
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
        ...
        <div className="ff" style={{ margin:0 }}>
          ...
          <select className="fi" value={reviewData.category || 'Others'}
        ...
        <div className="ff" style={{ margin:0 }}>
          ...
          <input className="fi" type="date" value={reviewData.issueDate || ''}
            onChange={e => setField('issueDate', e.target.value)} />
        </div>
        <div className="ff" style={{ gridColumn:'1/-1', margin:0 }}>
          <label className="fl">Expiry Date * {badge('expiryDate')}</label>
          <label style={{ display:'flex', alignItems:'center', gap:6, cursor:'pointer',
            fontSize:'0.74rem', color:'var(--cyan)', marginBottom:6 }}>
            <input type="checkbox" checked={!!reviewData.isUnlimited}
              onChange={e => setField('isUnlimited', e.target.checked)}
              style={{ accentColor:'var(--cyan)' }} />
            ∞ No Expiry / Unlimited Validity
          </label>
          {!reviewData.isUnlimited && (
            <input className="fi" type="date" value={reviewData.expiryDate || ''}
              onChange={e => setField('expiryDate', e.target.value)} />
          )}
        </div>
        {reviewData.issuingAuthority !== undefined && (
          <div className="ff" style={{ margin:0 }}>
  ...
  return (
    <div className="section">
      ...
      {/* ── Google Drive + AI Feature Banner ─────────────────────────────── */}
      <div style={{
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:'0.76rem', fontWeight:700,
            color: driveConnected ? 'var(--green)' : 'var(--cyan)',
            fontFamily:'Orbitron,monospace' }}>
            {driveConnected
              ? '✅ Google Drive Connected — 🤖 AI Certificate Scanner Active'
              : '☁️ Connect Google Drive to Enable AI Certificate Scanner'}
          </div>
          <div style={{ fontSize:'0.68rem', color:'var(--text3)', marginTop:3, lineHeight:1.5 }}>
            {driveConnected
              ? 'Upload any certificate photo or PDF → AI automatically reads certificate number, issue date & expiry date. Files are stored in your own Google Drive.'
          ...
          {driveConnected && (
            <div style={{ display:'flex', gap:12, marginTop:6, flexWrap:'wrap' }}>
              {[
                { icon:'📷', text:'Upload photo or PDF of certificate' },
                { icon:'🤖', text:'AI reads cert number, dates, title' },
                { icon:'✏️', text:'Review & edit any field instantly' },
                { icon:'☁️', text:'Saved to your Google Drive' },
              ].map((step, i) => (
                <span key={i} style={{ fontSize:'0.64rem', color:'var(--text3)',
        ...
        {driveConnected ? (
          <div style={{ display:'flex', gap:6, flexShrink:0 }}>
            <a href="https://drive.google.com/drive/folders" target="_blank" rel="noreferrer"
              style={{ padding:'5px 12px', borderRadius:7, fontSize:'0.7rem', fontWeight:700, cursor:'pointer',
      ...
      {showAdd && (
        <div style={{ background:'var(--card)', border:'1px solid rgba(0,180,216,0.3)',
          ...
          <div style={{ fontFamily:'Orbitron,monospace', fontSize:'0.78rem', color:'var(--cyan)', marginBottom:'0.8rem' }}>
            + Add New Certificate / Document
          </div>

          {/* ── NEW: AI Scan Banner in Add Form ─────────────────────────── */}
          <div style={{ background:'rgba(0,180,216,0.05)', border:'1px dashed rgba(0,180,216,0.35)',
            borderRadius:10, padding:'0.9rem', marginBottom:'1rem' }}>
            <div style={{ fontSize:'0.76rem', color:'var(--cyan)', fontWeight:700, marginBottom:3 }}>
              🤖 Smart Auto-Fill
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
              <input id="add-scan-input" type="file" accept=".pdf,.jpg,.jpeg,.png"
                style={{ display:'none' }}
              ...
              <label htmlFor="add-scan-input"
                style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'7px 16px',
                  borderRadius:8, cursor: scanningAdd ? 'default' : 'pointer', fontSize:'0.74rem', fontWeight:700,
                  background: scanningAdd ? 'rgba(0,180,216,0.08)' : 'rgba(0,180,216,0.15)',
                  color:'var(--cyan)', border:'1px solid rgba(0,180,216,0.45)',
                  pointerEvents: scanningAdd ? 'none' : 'auto', opacity: scanningAdd ? 0.7 : 1 }}>
                {scanningAdd
                  ? <><span className="spin" style={{ width:12, height:12, borderWidth:2 }} /> AI Reading…</>
                  : '📷 Scan Certificate'}
              </label>
              {addScanFile && !scanningAdd && (
                <span style={{ fontSize:'0.68rem', color:'var(--green)',
                  display:'flex', alignItems:'center', gap:6 }}>
                  ✅ {addScanFile.name}
                  {Object.keys(addAiFields).length > 0 &&
                    <span style={{ background:'rgba(0,180,216,0.15)', color:'var(--cyan)',
                      borderRadius:10, padding:'1px 7px', fontSize:'0.6rem', fontWeight:700 }}>
              ...
              {!addScanFile && (
                <span style={{ fontSize:'0.65rem', color:'var(--text3)' }}>or fill in manually below</span>
              )}
            </div>
          </div>
          {/* ─────────────────────────────────────────────────────────────── */}
          ...
          {renderFormFields(
            ...
            addAiFields   // ← pass AI-detected fields for badge display
          )}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ marginTop:8, display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                      {hasDriveFile ? (
                        <>
                          {/* File name badge */}
                          <span style={{ fontSize:'0.66rem', color:'var(--green)',
                          ...
                          {/* View */}
                          <button onClick={() => viewDriveFile(c.driveFileId)}
                            style={miniBtn('rgba(0,180,216,0.1)','var(--cyan)','1px solid rgba(0,180,216,0.3)')}>
                            📄 View
                          </button>

                          {/* Download */}
                          <button onClick={() => downloadDriveFile(c.driveFileId, c.driveFileName)}
                            style={miniBtn('rgba(100,200,100,0.1)','var(--green)','1px solid rgba(100,200,100,0.3)')}>
                            ⬇️ Download
                          </button>

                          {/* Update File (AI re-scan, marks as renewal) */}
                          <>
                            <input id={`upd-${c.id}`} type="file" accept=".pdf,.jpg,.jpeg,.png"
                              style={{ display:'none' }}
                              onChange={e => {
                                if (e.target.files?.[0]) handleFileUpload(c.id, e.target.files[0], true);
                                e.target.value = '';
                              }} />
                            <label htmlFor={`upd-${c.id}`}
                              style={{ ...miniBtn('rgba(240,165,0,0.1)','var(--gold)','1px solid rgba(240,165,0,0.35)'),
                          ...
                          {/* Remove */}
                          <button onClick={() => removeDriveFile(c.id)}
                            style={miniBtn('none','var(--text3)','1px solid rgba(255,255,255,0.1)')}>
                            ✕ Remove
                          </button>
                        </>
                      ) : driveConnected ? (
                        <>
                          {/* Upload + AI extract */}
                          <input id={`drv-${c.id}`} type="file" accept=".pdf,.jpg,.jpeg,.png"
                            style={{ display:'none' }}
                            onChange={e => {
                              if (e.target.files?.[0]) handleFileUpload(c.id, e.target.files[0], false);
                              e.target.value = '';
                            }} />
                        ...
                        <span style={{ fontSize:'0.64rem', color:'var(--text3)', fontStyle:'italic' }}>
                          Connect Drive above to upload certificate copy
                        </span>
                      )}
                    </div>
                    {/* ────────────────────────────────────────────────── */}
                  ...
                  <div style={{ borderTop:'1px solid rgba(0,180,216,0.25)', marginTop:12, paddingTop:12 }}>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                      ...
                      {!extracting && extractModal?.detected && Object.keys(extractModal.detected).length > 0 && (
                        <span style={{ fontSize:'0.62rem', color:'var(--green)',
                          background:'rgba(0,200,100,0.1)', borderRadius:10, padding:'2px 8px', fontWeight:700 }}>
                    ...
                    {extracting ? (
                      <div style={{ display:'flex', alignItems:'center', gap:10,
                        padding:'1.5rem', justifyContent:'center', color:'var(--cyan)' }}>
                        <div className="spin" />
                      ...
                      <>
                        {extractModal?.extractError && (
                          <div style={{ fontSize:'0.7rem', color:'#ff6b35', marginBottom:10,
                            background:'rgba(255,107,53,0.08)', borderRadius:8, padding:'6px 10px',
                            border:'1px solid rgba(255,107,53,0.25)' }}>
                            ⚠️ Auto-read failed — please fill in details manually below
                          </div>
                        )}
                        {extractModal?.isRenewal && (
                          <div style={{ fontSize:'0.7rem', color:'var(--gold)', marginBottom:10,
                            background:'rgba(240,165,0,0.08)', borderRadius:8, padding:'6px 10px',
                        ...
                        <div style={{ display:'flex', gap:6, marginTop:12, flexWrap:'wrap' }}>
                          <button className="btn btn-primary" style={{ fontSize:'0.72rem', padding:'6px 16px' }}
                            ...
                            {isUploading ? '⏳ Uploading to Drive…'
                              : driveToken ? '✅ Confirm & Upload to Drive' : '✅ Confirm & Save'}
                          </button>
                          <button
      ...
      <div className="info-box" style={{ marginTop:'1.2rem', fontSize:'0.72rem' }}>
        🔒 Certificate data stored in Firebase — visible only to you. Certificate files are stored in <strong>your own Google Drive</strong> — NavisphereX cannot access your other Drive files. AI extraction runs via Anthropic API and does not store your documents.
      </div>
    </div>
  );
}
