...
// NOTE: Contains temporary ScannerDiag component — remove after AI confirmed working
import { useState, useEffect } from "react";
import { db } from "../firebase";
import { doc, getDoc, setDoc } from "firebase/firestore";

// ─────────────────────────────────────────────────────────────────────────────
// TEMPORARY DIAGNOSTIC — shows scanner status visually on screen (mobile friendly)
// Remove <ScannerDiag /> from JSX and delete this component after AI is working
// ─────────────────────────────────────────────────────────────────────────────
function ScannerDiag() {
  ...
  const runTest = async () => {
    ...
    if (!key) {
      setStatus('fail');
      setDetail('REACT_APP_GEMINI_API_KEY is EMPTY.\n\nEnv var not loaded into the app.\n\nFix: Check Vercel → Settings → Environment Variables → confirm REACT_APP_GEMINI_API_KEY exists → Redeploy.');
      return;
    }
    setStatus('testing');
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus('ok');
        setDetail('API WORKING ✅\nModel replied: "' + reply.trim() + '"\n\nScanner is configured correctly.\nImage scanning should work.');
      } else {
        setStatus('fail');
        setDetail(
          'API ERROR ❌\n' +
  ...
  const colors = { idle: 'var(--cyan)', testing: '#ffa502', ok: 'var(--green)', fail: '#ff4757' };
  const labels = { idle: '▶ Run Scanner Test', testing: 'Testing…', ok: '✅ Scanner Working', fail: '❌ Scanner Error — see details' };
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '0.9rem', marginBottom: '1rem' }}>
      <div style={{ fontSize: '0.68rem', color: 'var(--text3)', fontFamily: 'Orbitron,monospace', marginBottom: 8 }}>
        🔧 SCANNER DIAGNOSTIC (admin only — remove after testing)
      </div>
      <div style={{ fontSize: '0.65rem', color: 'var(--text3)', marginBottom: 10 }}>
        Key in app: <code style={{ color: process.env.REACT_APP_GEMINI_API_KEY ? 'var(--green)' : '#ff4757', background: 'rgba(0,0,0,0.3)', padding: '1px 5px', borderRadius: 4 }}>
          {process.env.REACT_APP_GEMINI_API_KEY ? process.env.REACT_APP_GEMINI_API_KEY.substring(0, 14) + '…' : 'EMPTY — not loaded'}
        </code>
      </div>
      <button
        onClick={runTest}
        disabled={status === 'testing'}
        style={{ padding: '7px 16px', borderRadius: 8, cursor: 'pointer', fontSize: '0.74rem', fontWeight: 700, background: colors[status] + '22', color: colors[status], border: '1px solid ' + colors[status] + '55' }}
      >
        {labels[status]}
      </button>
      {detail && (
        <pre style={{ fontSize: '0.65rem', color: status === 'ok' ? 'var(--green)' : '#ff6b35', marginTop: 10, background: 'rgba(0,0,0,0.35)', borderRadius: 8, padding: '10px', whiteSpace: 'pre-wrap', wordBreak: 'break-all', fontFamily: 'monospace', lineHeight: 1.6 }}>
          {detail}
        </pre>
      )}
    </div>
...
function CertificateTrackerPage({ user, notify }) {
  ...
  const connectDrive = async () => {
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        script.onload = resolve; script.onerror = () => reject(new Error('Network error'));
      ...
      if (info.email && user?.email && info.email.toLowerCase() !== user.email.toLowerCase()) {
        notify(`Please use ${user.email} to connect storage.`, 'error'); setConnectingDrive(false); return;
      }
      ...
      if (!e.message?.includes('popup_closed') && !e.message?.includes('access_denied')) notify('Could not connect storage. Please try again.', 'error');
    }
    setConnectingDrive(false);
  };

  const disconnectDrive = () => {
    sessionStorage.removeItem('nsx_drive_token'); sessionStorage.removeItem('nsx_drive_expiry'); sessionStorage.removeItem('nsx_drive_email');
  ...
  const downloadDriveFile = async (id, name) => {
    if (!isDriveTokenValid()) { setDriveExpired(true); notify('Session expired — please reconnect storage', 'error'); return; }
    try {
      const blob = await driveGetBlob(driveToken, id);
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement('a');
      a.href = url; a.download = name||'certificate'; a.click(); URL.revokeObjectURL(url);
  ...
  const handleFileUpload = async (certId, file, isRenewal = false) => {
    ...
    try {
      ...
      const existing = certs.find(c => c.id===certId) || {};
      setExtractErrMsg(scanErrorMessage(e.message));
      setExtractModal({ certId, file, isRenewal, detected:{}, extractError:true,
  ...
  const confirmExtraction = async () => {
    ...
    try {
      ...
      await saveCerts(certs.map(c => c.id===certId ? { ...c, name:reviewData.name, certNo:reviewData.certNo, issueDate:reviewData.issueDate, expiryDate:finalExpiry, category:reviewData.category, notes:notesText, ...(driveFileId?{driveFileId,driveFileName}:{}) } : c));
      setExtractModal(null); setExtractErrMsg('');
      notify(driveFileId ? 'Certificate updated and file saved to your storage' : 'Certificate details updated', 'success');
  ...
  const handleAddFormScan = async (file) => {
    ...
    } catch(e) { console.error('[Scanner] Add-form:', e.message); setAddScanErrMsg(scanErrorMessage(e.message)); }
  ...
  const renderFormFields = (data,setData,isCN,setIsCN,isCC,setIsCC,isUnlim,setIsUnlim,aiFields={}) => {
    ...
    return (
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
        ...
        <div className="ff" style={{margin:0}}>
          ...
          {!isCC
            ?<select className="fi" value={data.category} onChange={e=>{if(e.target.value==='__cc__'){setIsCC(true);setData(n=>({...n,category:''}))}else setData(n=>({...n,category:e.target.value}))}}>
               ...
               <option value="__cc__">Custom...</option>
             </select>
            :<div style={{display:'flex',gap:6}}><input className="fi" placeholder="Custom category..." value={data.category} onChange={e=>setData(n=>({...n,category:e.target.value}))}/><button style={{background:'none',border:'1px solid var(--border)',borderRadius:6,color:'var(--text3)',cursor:'pointer',padding:'0 8px'}} onClick={()=>{setIsCC(false);setData(n=>({...n,category:'Others'}))}}>x</button></div>
          }
        </div>
        ...
        <div className="ff" style={{gridColumn:'1/-1',margin:0}}><label className="fl">Notes {badge('notes')}</label><input className="fi" placeholder="Issuing authority, endorsements..." value={data.notes} onChange={e=>setData(n=>({...n,notes:e.target.value}))}/></div>
      </div>
    );
  };
  ...
  const renderExtractionFields = () => {
    ...
    return (
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
        ...
        <div className="ff" style={{margin:0}}><label className="fl">Notes</label><input className="fi" value={reviewData.notes||''} placeholder="Additional notes..." onChange={e=>sf('notes',e.target.value)}/></div>
      </div>
    );
  };

  if (!user) return (
    <div className="section"><div className="empty"><div className="empty-icon">🔐</div><div className="empty-t">Login Required</div><div className="empty-d">Please log in to track your certificates.</div></div></div>
  );
    ...
    <div className="section">
      ...
      {driveExpired&&!driveConnected&&(
        <div style={{background:'rgba(255,107,53,0.08)',border:'1px solid rgba(255,107,53,0.35)',borderRadius:10,padding:'0.7rem 1rem',marginBottom:'0.8rem',display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap'}}>
          ...
          <button onClick={connectDrive} disabled={connectingDrive} style={{padding:'5px 14px',borderRadius:7,fontSize:'0.7rem',fontWeight:700,cursor:'pointer',background:'rgba(255,107,53,0.15)',color:'#ff6b35',border:'1px solid rgba(255,107,53,0.4)'}}>
      ...
      <div style={{background:driveConnected?'rgba(0,200,100,0.06)':'rgba(0,180,216,0.06)',border:`1px solid ${driveConnected?'rgba(0,200,100,0.25)':'rgba(0,180,216,0.25)'}`,borderRadius:12,padding:'0.75rem 1rem',marginBottom:'1rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:'0.76rem',fontWeight:700,color:driveConnected?'var(--green)':'var(--cyan)',fontFamily:'Orbitron,monospace'}}>{driveConnected?'Cloud Storage Connected':'Connect Cloud Storage'}</div>
          ...
          {!driveConnected&&<div style={{fontSize:'0.64rem',color:'var(--text3)',marginTop:4,fontStyle:'italic'}}>Scanning works without storage - connect only when you want to save files</div>}
        </div>
        {driveConnected
          ...
          :<button onClick={connectDrive} disabled={connectingDrive} style={{padding:'7px 18px',borderRadius:8,fontSize:'0.74rem',fontWeight:700,cursor:'pointer',background:'linear-gradient(135deg,#4285f4,#34a853)',color:'#fff',border:'none',whiteSpace:'nowrap',flexShrink:0,opacity:connectingDrive?0.7:1}}>
      ...
      <div className="sec-hdr">
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          {expiring.length>0&&<span style={{background:'rgba(255,71,87,0.15)',color:'#ff4757',border:'1px solid rgba(255,71,87,0.3)',borderRadius:20,padding:'3px 10px',fontSize:'0.7rem',fontWeight:700}}>{expiring.length} expiring</span>}
          <button style={{padding:'6px 14px',fontSize:'0.74rem',background:'rgba(0,180,216,0.1)',color:'var(--cyan)',border:'1px solid rgba(0,180,216,0.3)',borderRadius:8,cursor:'pointer'}} onClick={()=>{setShowExpDash(s=>!s);setActiveTier(null);}}>
            {showExpDash?'Close Timeline':'Expiry Timeline'}
          </button>
      ...
      {expiring.length>0&&<div style={{background:'rgba(255,71,87,0.08)',border:'1px solid rgba(255,71,87,0.25)',borderRadius:10,padding:'0.8rem 1rem',marginBottom:'1rem'}}>
        <div style={{fontFamily:'Orbitron,monospace',fontSize:'0.72rem',color:'#ff4757',marginBottom:'0.4rem'}}>ACTION REQUIRED</div>
      ...
      {showExpDash&&<div style={{background:'var(--card)',border:'1px solid rgba(0,180,216,0.2)',borderRadius:14,padding:'1.2rem',marginBottom:'1.2rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))',gap:8,marginBottom:'1rem'}}>
          {EXPIRY_TIERS.map(tier=>{const count=getCertsByTier(tier).length,isA=activeTier===tier.key;return(
            <div key={tier.key} onClick={()=>setActiveTier(isA?null:tier.key)} style={{background:isA?`${tier.color}22`:`${tier.color}0d`,border:`1px solid ${isA?tier.color:tier.color+'44'}`,borderRadius:10,padding:'0.75rem 0.5rem',cursor:'pointer',textAlign:'center'}}>
      ...
      {showAdd&&<div style={{background:'var(--card)',border:'1px solid rgba(0,180,216,0.3)',borderRadius:14,padding:'1.3rem',marginBottom:'1.2rem'}}>
        ...
        <div style={{background:'rgba(0,180,216,0.05)',border:'1px dashed rgba(0,180,216,0.35)',borderRadius:10,padding:'0.9rem',marginBottom:'1rem'}}>
          ...
          <div style={{fontSize:'0.68rem',color:'var(--text3)',marginBottom:10,lineHeight:1.5}}>Upload a photo or PDF - all details are filled in automatically.</div>
          <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
            ...
            <label htmlFor="add-scan-input" style={{display:'inline-flex',alignItems:'center',gap:6,padding:'7px 16px',borderRadius:8,cursor:scanningAdd?'default':'pointer',fontSize:'0.74rem',fontWeight:700,background:'rgba(0,180,216,0.15)',color:'var(--cyan)',border:'1px solid rgba(0,180,216,0.45)',pointerEvents:scanningAdd?'none':'auto',opacity:scanningAdd?0.7:1}}>
              {scanningAdd?'Reading...':'Scan Certificate'}
            </label>
            {addScanErrMsg&&!scanningAdd&&addScanFile&&<button onClick={retryAddScan} style={{padding:'5px 12px',borderRadius:7,fontSize:'0.7rem',fontWeight:700,cursor:'pointer',background:'rgba(0,180,216,0.1)',color:'var(--cyan)',border:'1px solid rgba(0,180,216,0.35)'}}>Try Again</button>}
            {addScanFile&&!scanningAdd&&!addScanErrMsg&&<span style={{fontSize:'0.68rem',color:'var(--green)',display:'flex',alignItems:'center',gap:6}}>
              {addScanFile.name}
              ...
              <button onClick={()=>{setAddScanFile(null);setAddAiFields({});setAddScanErrMsg('');setNewCert({...EMPTY_CERT});}} style={{background:'none',border:'none',color:'var(--text3)',cursor:'pointer'}}>x</button>
            </span>}
            {addScanErrMsg&&!scanningAdd&&<span style={{fontSize:'0.68rem',color:'#ff6b35'}}>{addScanErrMsg}</span>}
        ...
        {addScanFile&&!driveConnected&&<div style={{fontSize:'0.68rem',color:'var(--text3)',marginTop:8,background:'rgba(255,255,255,0.03)',borderRadius:8,padding:'6px 10px'}}>Certificate details will be saved. Connect storage to also save the file.</div>}
      ...
      {!loading&&filtered.length===0&&<div className="empty"><div className="empty-icon">📜</div><div className="empty-t">No Certificates Added Yet</div><div className="empty-d">Click Add Certificate to start tracking your STCW certificates.</div></div>}

      {filtered.length>0&&<div style={{display:'grid',gap:'0.7rem'}}>
        {filtered.map(c=>{
          const s=STATUS(c.expiryDate),isEd=editId===c.id,isQU=quickId===c.id,isUp=uploadingId===c.id,hasDr=!!c.driveFileId,isEx=extractModal?.certId===c.id;
          return(
          <div key={c.id} style={{background:'var(--card)',border:`1px solid ${s.color}33`,borderRadius:12,padding:'1rem',display:'flex',flexDirection:'column',gap:0}}>
            <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{marginTop:8,display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                  {hasDr?<>
                    <span style={{fontSize:'0.66rem',color:'var(--green)',background:'rgba(0,200,100,0.08)',border:'1px solid rgba(0,200,100,0.25)',borderRadius:20,padding:'2px 8px',maxWidth:190,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{c.driveFileName||'File saved'}</span>
                    ...
                    <button onClick={()=>downloadDriveFile(c.driveFileId,c.driveFileName)} style={mBtn('rgba(100,200,100,0.1)','var(--green)','1px solid rgba(100,200,100,0.3)')}>Download</button>
                    <><input id={`upd-${c.id}`} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{display:'none'}} onChange={e=>{if(e.target.files?.[0])handleFileUpload(c.id,e.target.files[0],true);e.target.value='';}}/><label htmlFor={`upd-${c.id}`} style={{...mBtn('rgba(240,165,0,0.1)','var(--gold)','1px solid rgba(240,165,0,0.35)'),cursor:'pointer',display:'inline-flex',alignItems:'center'}}>Update File</label></>
                  ...
                  </>:driveExpired?<button onClick={connectDrive} style={{fontSize:'0.64rem',padding:'3px 10px',borderRadius:6,cursor:'pointer',background:'rgba(255,107,53,0.08)',color:'#ff6b35',border:'1px solid rgba(255,107,53,0.3)'}}>Reconnect to upload</button>
                  :<span style={{fontSize:'0.64rem',color:'var(--text3)',fontStyle:'italic'}}>Connect storage to save file</span>}
                </div>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:4,flexShrink:0}}>
                <button onClick={()=>isQU?setQuickId(null):startQuick(c)} style={{background:isQU?'rgba(0,200,100,0.15)':'rgba(0,180,216,0.08)',border:`1px solid ${isQU?'rgba(0,200,100,0.4)':'rgba(0,180,216,0.3)'}`,color:isQU?'var(--green)':'var(--cyan)',borderRadius:6,cursor:'pointer',fontSize:'0.63rem',padding:'4px 8px',fontWeight:700,whiteSpace:'nowrap'}}>Renew</button>
                <button onClick={()=>isEd?cancelEdit():startEdit(c)} style={{background:isEd?'rgba(240,165,0,0.15)':'rgba(255,255,255,0.05)',border:`1px solid ${isEd?'rgba(240,165,0,0.5)':'rgba(255,255,255,0.1)'}`,color:isEd?'var(--gold)':'var(--text2)',borderRadius:6,cursor:'pointer',fontSize:'0.63rem',padding:'4px 8px',fontWeight:700}}>Edit</button>
                <button onClick={()=>deleteCert(c.id)} style={{background:'none',border:'1px solid rgba(255,71,87,0.25)',color:'#ff4757',borderRadius:6,cursor:'pointer',fontSize:'0.63rem',padding:'4px 8px'}}>Delete</button>
              </div>
            </div>

            {isEx&&<div style={{borderTop:'1px solid rgba(0,180,216,0.25)',marginTop:12,paddingTop:12}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10,flexWrap:'wrap',gap:6}}>
                <div style={{fontFamily:'Orbitron,monospace',fontSize:'0.7rem',color:'var(--cyan)'}}>{extracting?'READING CERTIFICATE...':'SCANNED DATA - Review and Edit'}</div>
              ...
              :<>
                {extractModal?.extractError&&<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:8,marginBottom:12,background:'rgba(255,107,53,0.08)',borderRadius:8,padding:'8px 10px',border:'1px solid rgba(255,107,53,0.25)'}}>
                  <span style={{fontSize:'0.7rem',color:'#ff6b35'}}>{extractErrMsg||'Could not read this file - please fill in details below'}</span>
                  <button onClick={retryExtraction} style={{padding:'4px 12px',borderRadius:6,fontSize:'0.7rem',fontWeight:700,cursor:'pointer',background:'rgba(255,107,53,0.15)',color:'#ff6b35',border:'1px solid rgba(255,107,53,0.4)',whiteSpace:'nowrap'}}>Try Again</button>
                </div>}
                {!driveConnected&&!driveExpired&&<div style={{fontSize:'0.68rem',color:'var(--text3)',marginBottom:10,background:'rgba(255,255,255,0.03)',borderRadius:8,padding:'6px 10px'}}>Details will be saved. Connect storage above to also save the file.</div>}
                {renderExtractionFields()}
                <div style={{display:'flex',gap:6,marginTop:12,flexWrap:'wrap'}}>
                  <button className="btn btn-primary" style={{fontSize:'0.72rem',padding:'6px 16px'}} onClick={confirmExtraction} disabled={saving||isUp}>{isUp?'Saving...':driveConnected?'Confirm and Save to Storage':'Confirm and Save Details'}</button>
                  <button style={{background:'none',border:'1px solid var(--border)',borderRadius:7,color:'var(--text3)',cursor:'pointer',fontSize:'0.72rem',padding:'5px 12px'}} onClick={()=>{setExtractModal(null);setExtractErrMsg('');}}>Cancel</button>
                </div>
              </>}
            </div>}

            {isQU&&<div style={{borderTop:'1px solid rgba(0,200,100,0.2)',marginTop:10,paddingTop:10}}>
            ...
            {isEd&&!isQU&&editData&&<div style={{borderTop:'1px solid rgba(240,165,0,0.25)',marginTop:10,paddingTop:10}}>
              <div style={{fontFamily:'Orbitron,monospace',fontSize:'0.68rem',color:'var(--gold)',marginBottom:8}}>EDIT CERTIFICATE</div>
