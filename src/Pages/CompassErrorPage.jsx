...
export default function CompassErrorPage({user}){
  ...
  const fmtDeviceLocal=d=>{try{return d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false})+' ('+Intl.DateTimeFormat().resolvedOptions().timeZone+')';}catch{return d.toLocaleTimeString();}};
  ...
  return(<div style={{padding:'1rem',maxWidth:980,margin:'0 auto',width:'100%'}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:'0.9rem',flexWrap:'wrap'}}>
      ...
      <div style={{marginLeft:'auto',display:'flex',gap:6,flexWrap:'wrap'}}>
        ...
        <button onClick={()=>setShowHelp(h=>!h)} style={{padding:'3px 10px',borderRadius:100,fontSize:'0.62rem',background:'rgba(0,180,216,0.1)',border:'1px solid rgba(0,180,216,0.3)',color:'var(--cyan)',cursor:'pointer',fontWeight:600}}>
          {showHelp?'✕ Close':'❓ How to Use'}
        </button>
      </div>
    </div>
    ...
    {/* HOW TO USE PANEL */}
    {showHelp&&(<div style={{background:'var(--card)',border:'1px solid rgba(0,180,216,0.3)',borderRadius:12,padding:'1.1rem',marginBottom:'0.8rem',borderLeft:'3px solid var(--cyan)'}}>
      <div style={{fontFamily:"'Orbitron',monospace",fontSize:'0.75rem',fontWeight:700,color:'var(--cyan)',marginBottom:'0.8rem',letterSpacing:'0.06em'}}>🧭 HOW TO USE — AUTO MODE (3 STEPS)</div>
      <div style={{display:'flex',flexDirection:'column',gap:'0.7rem'}}>
        {[
          {n:'1',title:'GPS Auto-Lock',color:'var(--green)',steps:[
            'Open page → GPS locks automatically. Badge shows 📍 Position Locked.',
            'If not locked, tap 🔄 Refresh Lock button.',
          ...
          {n:'3',title:'Enter Gyro Bearing of Body',color:'var(--cyan)',steps:[
            'Select celestial body from dropdown. Green ✓ = best altitude. Orange ◦ = usable.',
            'The gyro bearing field is pre-filled with the computed true bearing as a guide.',
            'Observe body through azimuth mirror. Enter the GYRO bearing you read on the gyro compass.',
            'Adjust to nearest whole number by typing directly or use ‹ › buttons for ±0.1° fine adjustment.',
            'Gyro Error, Standard Error, and Deviation appear instantly in the output card.',
          ]},
        ].map(({n,title,color,steps})=>(
          <div key={n} style={{display:'flex',gap:'0.7rem'}}>
            <div style={{width:24,height:24,borderRadius:'50%',background:color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.72rem',fontWeight:900,color:'#0a1628',flexShrink:0,marginTop:2}}>{n}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:'0.78rem',fontWeight:700,color:color,marginBottom:'0.3rem'}}>{title}</div>
              <ul style={{paddingLeft:'1rem',margin:0}}>
                {steps.map((s,i)=><li key={i} style={{fontSize:'0.72rem',color:'var(--text2)',lineHeight:1.7,marginBottom:2}}>{s}</li>)}
              </ul>
            </div>
          </div>
        ))}
      </div>
      <div style={{marginTop:'0.8rem',padding:'8px 10px',background:'rgba(0,180,216,0.06)',border:'1px solid rgba(0,180,216,0.2)',borderRadius:8,fontSize:'0.68rem',color:'var(--text3)',lineHeight:1.6}}>
        💡 <strong style={{color:'var(--text2)'}}>Note on time:</strong> Nautical LMT = UTC + longitude/15 hours (standard navigation). Device time = your phone/tablet timezone. Calculations always use locked UTC.
      </div>
    </div>)}
    ...
    <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:12,padding:'0.9rem',marginBottom:'0.8rem'}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:8}}>
        <div style={{fontFamily:'monospace',fontSize:'0.78rem',lineHeight:2}}>
          ...
          {calcPos&&<div style={{color:'var(--cyan)'}}>{fmtLMT(calcTime||displayNow,calcPos.lon)} <span style={{fontSize:'0.68rem',color:'var(--text3)',marginLeft:8}}>· {fmtDeviceLocal(displayNow)}</span></div>}
