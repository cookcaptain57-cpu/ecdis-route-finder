...
<div style={{fontSize:‘0.7rem’,color:‘var(–text2)’}}>{p.city&&p.city!==p.name?${p.city} · :’’}{p.country}&lt;/div&gt; &lt;/div&gt; &lt;span style={{background:‘rgba(0,180,216,0.1)’,color:‘var(–cyan)’,border:‘1px solid rgba(0,180,216,0.2)’, borderRadius:5,padding:‘1px 7px’,fontSize:‘0.63rem’,fontFamily:‘monospace’,flexShrink:0}}&gt;{p.id}&lt;/span&gt; &lt;/div&gt; ))} &lt;/div&gt; )} &lt;/div&gt; {selected?( &lt;div style={{background:‘var(–card)’,border:‘1px solid rgba(0,180,216,0.3)’,borderRadius:16,padding:‘1.4rem’,boxShadow:‘0 8px 32px rgba(0,0,0,0.4)’}}&gt; &lt;div style={{display:‘flex’,justifyContent:‘space-between’,alignItems:‘flex-start’,marginBottom:‘1rem’,flexWrap:‘wrap’,gap:8}}&gt; &lt;div&gt; &lt;div style={{fontFamily:‘Orbitron,monospace’,fontSize:‘1rem’,fontWeight:700,color:‘var(–cyan)’,marginBottom:4}}&gt;{selected.name}&lt;/div&gt; &lt;div style={{fontSize:‘0.8rem’,color:‘var(–text2)’}}&gt;{selected.city&&selected.city!==selected.name?``${selected.city} ·:’’}{selected.country}</div>
</div>
