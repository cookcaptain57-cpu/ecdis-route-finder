...
// ─── AI System Prompt ─────────────────────────────────────────────────────────
const SYS = `You are NavisphereX Marine AI — a knowledgeable maritime assistant helping seafarers and maritime professionals with any maritime question.

You can answer anything related to maritime:
- Regulations: SOLAS, MARPOL, STCW, MLC 2006, COLREGS, ISM, ISPS, IMDG
- Navigation: celestial nav, chart work, passage planning, anchoring, watchkeeping
- Ship operations: cargo, ballast, bunkering, mooring, stability
- Safety & emergencies: firefighting, LSA, MOB, distress signals
- Ship types: tankers, bulk carriers, container ships, gas carriers
- Port/flag state: PSC inspections, surveys, certificates
- Crew: rest hours, medical, welfare, sea time
- Weather: tropical cyclones, ocean currents, tidal streams
- Equipment: ECDIS, radar, AIS, GMDSS, compasses

Response style:
- Direct and practical — mariners need fast accurate answers
- Cite specific regulations when relevant (e.g. SOLAS Ch.III Reg.7)
- Use bullet points for lists
- Under 350 words
- For safety-critical topics add a brief note to verify against official publications`;
  ...
  const formatMsg = (text) => text.split('\n').map((line, i) => {
    ...
    return (
      <div key={i} style={{ marginBottom:2 }}>
        {parts.map((p, j) => j%2===1 ? <strong key={j} style={{ color:'var(--cyan)' }}>{p}</strong> : p)}
      </div>
    );
  });
    ...
    <div style={{ marginBottom:'1.4rem' }}>
        ...
        <div onClick={() => setIsOpen(true)}
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontFamily:'Orbitron,monospace', fontSize:'0.72rem', fontWeight:700,
              color:'var(--green)', marginBottom:4 }}>Ask Anything Maritime</div>
      ...
      {isOpen && (
        <div style={{ background:'var(--card)', border:'1px solid rgba(0,200,150,0.3)',
          borderRadius:16, overflow:'hidden', boxShadow:'0 12px 40px rgba(0,0,0,0.5)' }}>
              ...
              {messages.length > 0 && (
                <button onClick={clearChat}
                  style={{ background:'rgba(255,71,87,0.1)', border:'1px solid rgba(255,71,87,0.25)',
                    ...
                    cursor:'pointer', fontFamily:'Exo 2,sans-serif' }}>Clear</button>
              )}
              <button onClick={() => setIsOpen(false)}
                style={{ background:'rgba(255,255,255,0.06)', border:'1px solid var(--border)',
                  color:'var(--text2)', borderRadius:7, padding:'4px 9px', fontSize:'0.8rem', cursor:'pointer' }}>✕</button>
            </div>
          </div>
          ...
          <div style={{ height:320, overflowY:'auto', padding:'12px 14px',
            ...
            {messages.length === 0 && !loading && (
              <div style={{ textAlign:'center', paddingTop:20, paddingBottom:10 }}>
                ...
                <div style={{ color:'var(--text3)', fontSize:'0.68rem', marginBottom:18 }}>
                  Navigation · Regulations · Procedures · Equipment
                </div>
            ...
            {messages.map((msg, idx) => (
              <div key={idx} style={{ marginBottom:14, display:'flex', flexDirection:'column',
                alignItems: msg.role==='user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ fontSize:'0.58rem', fontWeight:700, letterSpacing:'0.1em',
                  color: msg.role==='user' ? 'var(--cyan)' : 'var(--green)', marginBottom:4,
                ...
                <div style={{ maxWidth:'88%', position:'relative',
                  ...
                  border: msg.role==='user'
                    ? '1px solid rgba(0,180,216,0.3)' : '1px solid rgba(0,200,150,0.15)',
                  ...
                  {msg.role==='assistant' && (
                    <button onClick={() => copyText(msg.content, idx)}
                      style={{ position:'absolute', top:6, right:6,
                        ...
                        borderRadius:5, padding:'2px 6px', fontSize:'0.58rem', cursor:'pointer',
            ...
            {loading && (
              <div style={{ marginBottom:14, display:'flex', flexDirection:'column', alignItems:'flex-start' }}>
                <div style={{ fontSize:'0.58rem', fontWeight:700, letterSpacing:'0.1em',
                  color:'var(--green)', marginBottom:4, paddingLeft:2 }}>⚓ NAVISPHERE AI</div>
            ...
            <div style={{ display:'flex', gap:8, alignItems:'flex-end',
              ...
              <textarea
                ...
                style={{ flex:1, background:'transparent', border:'none', outline:'none',
                  ...
                  fontFamily:'Exo 2,sans-serif', maxHeight:80, overflowY:'auto', scrollbarWidth:'none' }}
                onInput={e => { e.target.style.height='auto'; e.target.style.height=Math.min(e.target.scrollHeight,80)+'px'; }}
              />
              <button onClick={() => sendMessage()} disabled={!input.trim() || loading}
                style={{ width:34, height:34, flexShrink:0, borderRadius:9, border:'none',
                  background: input.trim()&&!loading ? 'linear-gradient(135deg,#00C896,#00a87a)' : 'rgba(255,255,255,0.06)',
...
export default function HomePage({ routes, charts, onSearch, setTab, user, portsDb=[], userProfile=null }) {
  ...
  const loadSeaTime = async () => {
    try {
      const s = await getDoc(doc(db,'seatime',user.uid));
      if (s.exists()) setSeaTimeDays((s.data().entries||[]).reduce((t,e)=>t+calcDays(e.signOn,e.signOff),0));
    } catch {}
  };

  const loadCerts = async () => {
    try {
      const s = await getDoc(doc(db,'certificates',user.uid));
    ...
    <div style={{ background:'var(--bg)', minHeight:'calc(100vh - 56px)', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ position:'relative', background:'linear-gradient(135deg,#020810 0%,#040C1A 40%,#071428 70%,#0a1e3a 100%)', overflow:'hidden', padding:'2rem 1.5rem 2.5rem', borderBottom:'1px solid var(--border)' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', gap:'1.5rem', flexWrap:'wrap' }}>
          ...
          {/* LEFT: title + search */}
          <div ref={wRef} style={{ flex:1, minWidth:240, position:'relative' }}>
                        ...
                        {items.map((r,i) => (
                          <div key={i} onMouseDown={() => { setQ(''); setQResults([]); onSearch(r.label); }}
                            ...
                            <div>
                              ...
                              {r.sub && <div style={{ fontSize:'0.66rem', color:'var(--text3)' }}>{r.sub}</div>}
                            </div>
                          </div>
                        ))}
                      </div>
          ...
          <div style={{ flexShrink:0, width:180, background:'rgba(7,20,40,0.7)', border:'1px solid rgba(0,180,216,0.2)', borderRadius:14, padding:'0.9rem', backdropFilter:'blur(10px)' }}>
            ...
            {weatherLoading && (
              <div style={{ textAlign:'center', padding:'0.4rem 0' }}>
                <div className="spin" style={{ width:14, height:14, margin:'0 auto' }} />
              </div>
            )}
            ...
            {!weather && !weatherLoading && (
              <div style={{ fontSize:'0.66rem', color:'var(--text3)', textAlign:'center', padding:'0.3rem 0' }}>📍 or search a port</div>
            )}
          </div>
      ...
      <div style={{ padding:'1.4rem 1.2rem', maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginBottom:'1.4rem' }}>
            {seaTimeDays!==null && (
              <div onClick={()=>setTab('seatime')} style={{ background:'var(--card)', border:'1px solid rgba(0,180,216,0.25)', borderRadius:12, padding:'10px 14px', cursor:'pointer', display:'flex', alignItems:'center', gap:10, flex:'1', minWidth:140 }}>
                ...
                <div>
                  <div style={{ fontFamily:'Orbitron,monospace', fontSize:'0.82rem', fontWeight:700, color:'var(--cyan)' }}>{fmt(seaTimeDays)}</div>
                  <div style={{ fontSize:'0.6rem', color:'var(--text3)', textTransform:'uppercase' }}>Sea Time</div>
                </div>
              </div>
            )}
            {expCerts.length>0 && (
              <div onClick={()=>setTab('certs')} style={{ background:'rgba(255,71,87,0.06)', border:'1px solid rgba(255,71,87,0.3)', borderRadius:12, padding:'10px 14px', cursor:'pointer', display:'flex', alignItems:'center', gap:10, flex:'1', minWidth:140 }}>
                <div style={{ fontSize:'1.6rem' }}>📜</div>
                <div>
                  <div style={{ fontFamily:'Orbitron,monospace', fontSize:'0.82rem', fontWeight:700, color:'#ff4757' }}>{expCerts.length} Expiring</div>
                  <div style={{ fontSize:'0.6rem', color:'#ff4757', textTransform:'uppercase' }}>Certificates</div>
                </div>
              </div>
            )}
            {isOffline && (
              <div style={{ background:'rgba(0,200,100,0.06)', border:'1px solid rgba(0,200,100,0.25)', borderRadius:12, padding:'10px 14px', display:'flex', alignItems:'center', gap:10, minWidth:120 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:'0.6rem', color:'var(--green)', textTransform:'uppercase' }}>Offline</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{ marginBottom:'0.6rem', display:'flex', alignItems:'center', gap:10 }}>
          ...
          <button onClick={() => setShowTabSettings(true)}
            style={{ background:'rgba(0,180,216,0.08)', border:'1px solid rgba(0,180,216,0.25)', borderRadius:7, padding:'4px 9px', cursor:'pointer', fontSize:'0.72rem', color:'var(--cyan)', display:'flex', alignItems:'center', gap:5, fontFamily:'Exo 2,sans-serif' }}>
            ⚙️ Customise
          </button>
        </div>
        ...
        {/* Customise modal */}
        {showTabSettings && (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', zIndex:9990, display:'flex', alignItems:'flex-end', justifyContent:'center' }}
            ...
            <div style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:'18px 18px 0 0', padding:'1.4rem', width:'100%', maxWidth:600, maxHeight:'80vh', overflowY:'auto' }}>
              ...
              <button className="btn btn-primary" style={{ width:'100%', justifyContent:'center', marginTop:'1rem' }}
                onClick={() => setShowTabSettings(false)}>✅ Done</button>
            </div>
          </div>
        )}
        ...
        {user && (
          <div onClick={() => setTab('account')}
            style={{ background:'linear-gradient(135deg,var(--card) 0%,#0F2444 100%)', border:'1px solid rgba(0,180,216,0.2)', borderRadius:16, padding:'1.2rem', cursor:'pointer', display:'flex', alignItems:'center', gap:16, marginBottom:'1.4rem', transition:'all 0.2s', position:'relative', overflow:'hidden' }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))', gap:8, marginBottom:'1.4rem' }}>
          {[
            ...
            <button key={i} onClick={() => setTab(a.tab)}
              style={{ padding:'10px', borderRadius:10, border:`1px solid ${a.color}33`, background:`${a.color}0a`, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'flex-start', gap:4, fontFamily:'Exo 2,sans-serif', transition:'all 0.2s' }}
        ...
        <div style={{ display:'flex', gap:8, overflowX:'auto', paddingBottom:8, marginBottom:'1.4rem', scrollbarWidth:'none' }}>
          {KNOWLEDGE.map((k,i) => (
            <div key={i} onClick={() => setTab(k.tab)}
              style={{ flexShrink:0, width:130, background:'var(--card)', border:'1px solid var(--border)', borderRadius:12, padding:'1rem', cursor:'pointer', transition:'all 0.2s' }}
