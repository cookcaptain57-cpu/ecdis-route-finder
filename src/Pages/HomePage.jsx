...
function HomePage({ routes, charts, onSearch, setTab, user, portsDb = [] }) {
  ...
  useEffect(() => {
    ...
    [...routes, ...charts].forEach(f =>
      [f.fileName, f.portName, f.keywords, f.brand].filter(Boolean).forEach(s => {
  ...
  const doSearch = (val) => {
    const v = val || q;
    if (v.trim()) { onSearch(v); setShowSugg(false); }
  };

  // ✅ FIXED: all var(–x) → var(--x) throughout FEATURE_CARDS
  const FEATURE_CARDS = [
  ...
  // ✅ FIXED: all var(–x) → var(--x) throughout QUICK_ACTIONS
  const QUICK_ACTIONS = [
  ...
  // ✅ FIXED: all var(–x) → var(--x) throughout KNOWLEDGE
  const KNOWLEDGE = [
    ...
    <div style={{ flex: 1 }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '0.4rem' }}>
          <h1 style={{
            fontFamily: 'Orbitron,monospace', fontSize: 'clamp(1.4rem,5vw,2.4rem)', fontWeight: 900,
            lineHeight: 1.1, letterSpacing: '0.04em', marginBottom: '0.3rem'
          }}>
            NAVISPHERE<span style={{ color: 'var(--cyan)' }}>X</span> MARINE
          </h1>
          <div style={{
            display: 'flex', gap: '1rem', flexWrap: 'wrap', fontSize: '0.68rem',
      ...
      <div style={{ padding: '1.4rem', maxWidth: 1100, margin: '0 auto', width: '100%' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '0.8rem' }}>
            {FEATURE_CARDS.map((c, i) => (
              <div key={i} onClick={() => setTab(c.tab)}
                ...
                {c.badge && (
                  <span style={{
                    position: 'absolute', top: 10, right: 10, background: c.color, color: '#000',
                ...
                <div style={{
                  ...
                  color: c.color, marginBottom: '0.4rem', letterSpacing: '0.05em'
                }}>{c.title}</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text2)', lineHeight: 1.5, marginBottom: '0.8rem' }}>{c.desc}</div>
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <span style={{
                    width: 28, height: 28, borderRadius: '50%', border: `1px solid ${c.color}55`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center', color: c.color, fontSize: '0.8rem'
                  }}>→</span>
                </div>
              </div>
            ))}
        ...
        {/* MY ACCOUNT */}
        <div onClick={() => setTab(user ? 'home' : 'login')} style={{
          background: 'linear-gradient(135deg,rgba(11,29,53,1),rgba(15,36,68,0.8))',
          border: '1px solid var(--border)', borderRadius: 14, padding: '1.2rem 1.4rem', cursor: 'pointer',
          marginBottom: '1.6rem', display: 'flex', alignItems: 'center', gap: 14, transition: 'all 0.2s'
        }}
          onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--cyan)'}
          onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
          <div style={{
            width: 48, height: 48, borderRadius: '50%', background: 'rgba(0,180,216,0.1)',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: '0.7rem' }}>
            {QUICK_ACTIONS.map((a, i) => (
              <div key={i} onClick={() => setTab(a.tab)}
                ...
                <div style={{
                  width: 38, height: 38, borderRadius: 10, background: 'rgba(0,0,0,0.2)',
            ...
            {KNOWLEDGE.map((k, i) => (
              <div key={i} onClick={() => setTab('library')}
                ...
                <div style={{
                  ...
                  color: k.color, marginBottom: 4, letterSpacing: '0.04em'
                }}>{k.title}</div>
