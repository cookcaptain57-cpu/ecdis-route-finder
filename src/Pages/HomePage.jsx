...
const REST_PERIODS = [
  { label: "Min rest in any 24h period", minRest: 10 },
  { label: "Min rest in any 7-day period", minRest: 77 },
];
...
function FatigueAssessment() {
  ...
  const totalRest7 = entries.reduce((s, e) => s + (parseFloat(e.rest24) || 0), 0);
  const violations = entries.map(e => {
    const r = parseFloat(e.rest24) || 0;
    return r > 0 && r < 10;
  });
  const week7Viol = totalRest7 < 77 && entries.some(e => e.rest24 !== "");
    ...
    <div className="cw-card">
      ...
      {entries.map((e, i) => (
        <div className="rest-row" key={i}>
          <span style={{fontSize:"0.72rem",color:violations[i]?"var(--red)":"var(--text)"}}>{e.day} {violations[i]?"⚠️":""}</span>
          <input className="fi" type="number" min="0" max="24" placeholder="hrs"
            value={e.rest24} onChange={v => update(i, "rest24", v.target.value)}
            style={{padding:"4px 7px",fontSize:"0.74rem"}} />
          <input className="fi" type="number" min="0" max="24" placeholder="hrs"
            value={e.work24} onChange={v => update(i, "work24", v.target.value)}
            style={{padding:"4px 7px",fontSize:"0.74rem"}} />
        </div>
      ))}
      <div className="cw-result">
...
function FullStrategySavingPlanner() {
  ...
  const contractsNeeded   = gAmt > 0 ? Math.ceil(gAmt / savingPerContract) : 0;
