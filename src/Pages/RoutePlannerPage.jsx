...
<div style={{
display: ‘flex’, alignItems: ‘center’, gap: 8, padding: ‘0.7rem 1rem’,
background: ‘var(–card)’, borderBottom: ‘1px solid var(–border)’, flexWrap: ‘wrap’
}}>
<input className=“fi”
style={{ flex: 1, minWidth: 150, padding: ‘7px 12px’, fontSize: ‘0.82rem’ }}
placeholder=“Route Name…”
value={routeName}
onChange={e => setRouteName(e.target.value)} />
{totalNM > 0 && (
<span style={{ fontFamily: ‘Orbitron,monospace’, fontSize: ‘0.78rem’, color: ‘var(–cyan)’, whiteSpace: ‘nowrap’ }}>
📏 {totalNM.toFixed(1)} NM
</span>
)}
<span style={{ fontFamily: ‘Orbitron,monospace’, fontSize: ‘0.72rem’, color: ‘var(–text2)’ }}>
{waypoints.length} WPTs
</span>
<button className=“btn btn-gold” style={{ padding: ‘7px 12px’, fontSize: ‘0.72rem’ }}
disabled={waypoints.length < 2}
onClick={() => downloadFile(exportRTZ(routeName, waypoints), `${routeName.replace(/\s+/g, '-')}.rtz`, ‘application/xml’)}>
⬇ RTZ
</button>
<button className=“btn btn-green” style={{ padding: ‘7px 12px’, fontSize: ‘0.72rem’ }}
disabled={waypoints.length < 2}
onClick={() => downloadFile(exportCSV(waypoints), `${routeName.replace(/\s+/g, '-')}.csv`, ‘text/csv’)}>
⬇ CSV
</button>
<button className=“btn btn-danger” style={{ padding: ‘7px 12px’, fontSize: ‘0.72rem’ }} onClick={clearRoute}>
🗑 Clear
</button>
{/* Map Mode Toggle */}
...
style={{
padding: ‘5px 10px’, fontSize: ‘0.68rem’, border: ‘none’, cursor: ‘pointer’,
        ...
        {panel === 'auto' && (
          <>
                      ...
                      <div key={p.id} className="ac-item" onClick={() => { setFromPort(p.name); setFromSugg([]); }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.62rem', color: 'var(--text3)' }}>
                            {p.lat.toFixed(4)}°N / {p.lon.toFixed(4)}°E · {p.id}
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            {showDbSugg && dbSuggestions.length > 0 && (
              <div style={{
                marginBottom: '1rem', background: 'rgba(0,180,216,0.06)',
                border: '1px solid rgba(0,180,216,0.25)', borderRadius: 10, padding: '10px'
              }}>
                ...
                {dbSuggestions.map((r, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      background: 'rgba(0,0,0,0.2)', marginBottom: 5, cursor: 'pointer',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{
                          fontSize: '0.8rem', fontWeight: 600, overflow: 'hidden',
                      ...
                      <button style={{
                        ...
                        padding: '4px 10px', fontSize: '0.68rem', fontWeight: 700, cursor: 'pointer', flexShrink: 0
                      }}>
                        USE
                      </button>
                    </div>
                ...
                <button className="btn btn-secondary"
                  style={{ width: '100%', fontSize: '0.7rem', padding: '5px', marginTop: 4 }}
            ...
            <div className="p-section">
              ...
              <button className={`btn ${clickAdd ? 'btn-gold' : 'btn-secondary'}`}
              ...
              <div className="overlay-grid">
                {ovCfg.map(ov => (
                  <button key={ov.k} className={`ov-btn ${overlays[ov.k] ? 'active' : ''}`}
                    style={{
                      color: overlays[ov.k] ? ov.color : 'var(--text2)',
                      borderColor: overlays[ov.k] ? ov.color : 'var(--border)'
                    }}
            ...
            <div className="p-section">
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8 }}>
                <button className={`btn ${playing ? 'btn-danger' : 'btn-green'}`}
        ...
        {panel === 'load' && (
          <>
            <div className="p-section">
              ...
              <div style={{
                border: '2px dashed var(--border2)', borderRadius: 10, padding: '1.5rem',
                textAlign: 'center', cursor: 'pointer', background: 'var(--bg2)', marginBottom: '0.8rem'
              }}>
                ...
                <input type="file" accept=".rtz,.rtzp" onChange={handleRTZLoad}
                  style={{ display: 'block', marginTop: 10, width: '100%', fontSize: '0.75rem' }} />
              </div>
              {waypoints.length > 0 && (
                <div className="ok-box" style={{ textAlign: 'center', fontSize: '0.78rem' }}>
                  ✅ {waypoints.length} waypoints loaded from file
                </div>
              )}
            </div>
            <div className="p-section">
        ...
        {panel === 'wpts' && (
          <>
            <div style={{ marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {waypoints.length > 0 && (
                <button className="btn btn-danger" style={{ padding: '4px 9px', fontSize: '0.7rem' }} onClick={clearRoute}>
                  Clear All
                </button>
              )}
            </div>
            {waypoints.length === 0
              ? (
                <div className="empty">
                  ...
                  <div className="empty-t">No Waypoints</div>
                  <div className="empty-d">Generate a route or load an RTZ file</div>
                </div>
              ) : (
                <div style={{ overflowX: 'auto' }}>
                  <table className="wp-table">
                    <thead>
                      <tr><th>WP</th><th>Lat</th><th>Lon</th><th>Crs°</th><th>NM</th><th>Del</th></tr>
                    </thead>
                    <tbody>
                      {waypoints.map((wp, i) => (
                        <tr key={i}>
                          <td style={{ color: 'var(--cyan)', fontFamily: 'Orbitron,monospace' }}>
                          ...
                          <td>
                            <button onClick={() => removeWP(i)}
                              style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: '0.9rem' }}>
                              ✕
                            </button>
                          </td>
                        </tr>
                  ...
                  <div style={{
                    marginTop: 8, padding: '8px', background: 'var(--bg2)', borderRadius: 8,
                    textAlign: 'center', fontFamily: 'Orbitron,monospace', fontSize: '0.76rem', color: 'var(--gold)'
                  }}>
