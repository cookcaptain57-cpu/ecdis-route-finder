...
// ← ADDED: zone constants for route safety check
...
function RoutePlannerPage({ notify, sheetRoutes = [], portsDb = [] }) {
  ...
  const [playing,     setPlaying]     = useState(false);   // kept — still passed to MapView
  const [speed,       setSpeed]       = useState(5);       // kept — still passed to MapView
  ...
  const [mapMode,     setMapMode]     = useState('day');   // ← CHANGED: default now 'day'
  const [dbSuggestions, setDbSuggestions] = useState([]);
  const [showDbSugg,  setShowDbSugg]  = useState(false);

  // ← ADDED: new state for manual route builder
  ...
  const totalManualNM = useMemo(() => totalRouteNM(manualWps),  [manualWps]); // ← ADDED

  // ← ADDED: convert check results into MapView highlight descriptors
  const checkHighlights = useMemo(() => {
    return checkResults
      ...
      .flatMap(r => {
        ...
        if (r.type === 'land') {
          // segIdx is 1-based segment index (leg from WP[segIdx-1] to WP[segIdx])
          if (r.segIdx > 0 && manualWps[r.segIdx - 1] && manualWps[r.segIdx]) {
            return [{
              type: 'segment',
          ...
          // segIdx is 0-based waypoint index
          if (manualWps[r.segIdx]) {
            return [{
              type: 'point',
              lat: manualWps[r.segIdx].lat, lon: manualWps[r.segIdx].lon,
              color, message: r.message, severity: r.severity,
            }];
          }
        }
  ...
  // ── Persistence: restore all state on mount ──────────────────────────────
  useEffect(() => {
    const load = async () => {
      try {
        // localStorage — simple scalars
  ...
  useEffect(() => { idbSavePref('mnp_waypoints',  waypoints).catch(()=>{}); },       [waypoints]);
  useEffect(() => { idbSavePref('mnp_routeName',  routeName).catch(()=>{}); },       [routeName]);
  useEffect(() => { idbSavePref('mnp_manualWps',  manualWps).catch(()=>{}); },       [manualWps]);
  useEffect(() => { idbSavePref('mnp_manualRName',manualRouteName).catch(()=>{}); }, [manualRouteName]);

  // ← ADDED: clear stale check results when WP count changes (structural edit)
  useEffect(() => { setCheckResults([]); }, [manualWps.length]);

  // ── Existing port search logic (unchanged) ────────────────────────────────
  ...
  // ← CHANGED: added manual panel case at top — clicking always adds WP in manual mode
  ...
  const saveManualRoute = async () => {
    ...
    const route = {
      id: `route_${Date.now()}`,
      name: manualRouteName || 'Manual Route',
      waypoints: manualWps,
      savedAt: new Date().toISOString(),
      totalNM: totalManualNM,
    };
    try {
      ...
      notify('Failed to save — storage error', 'error');
    }
  };
  ...
  const deleteSavedRoute = async (id) => {
    try {
      ...
      notify('Route deleted from library', 'success');
    } catch (e) {
  ...
  const exportManualRoute = () => {
    if (manualWps.length < 2) { notify('Add at least 2 waypoints first', 'error'); return; }
    const name = manualRouteName || 'Manual Route';
  ...
  // ← ADDED: async route safety check
  const performRouteCheck = async () => {
    ...
    try {
      // 1. Duplicate / too-close waypoints
      for (let i = 1; i < manualWps.length; i++) {
        if ((manualWps[i].distance || 0) < 0.1) {
          results.push({ segIdx: i, type: 'duplicate', severity: 'error',
            message: `WP${i} & WP${i+1} are duplicate or too close (< 0.1 NM) — remove one` });
        }
      }

      // 2. Sharp course changes (> 140°)
      for (let i = 2; i < manualWps.length; i++) {
        const b1 = manualWps[i-1].bearing || 0, b2 = manualWps[i].bearing || 0;
        let diff = Math.abs(b2 - b1);
        if (diff > 180) diff = 360 - diff;
        if (diff > 140) {
          results.push({ segIdx: i, type: 'sharpTurn', severity: 'warning',
            message: `WP${i+1}: sharp course change of ${diff.toFixed(0)}° — may be impractical in heavy weather` });
        }
      }

      // 3. Zone crossings — piracy, ECA, SECA, MARPOL, anchorage
      const allZones = [
        ...
        allZones.forEach(zone => {
          ...
          if (!seen.has(key) && pointInPolygon(wp.lat, wp.lon, coords)) {
            ...
            results.push({ segIdx: i, type: zone.ztype, severity: zone.sev,
              message: `WP${i+1} is inside ${zone.label}: ${zone.name}` });
          }
        });
      });

      // 4. Land crossing check via GEBCO elevation API (batch, 1-based segment index)
      const midpoints = manualWps.slice(1).map((wp, idx) => ({
        lat: (manualWps[idx].lat + wp.lat) / 2,
        lon: (manualWps[idx].lon + wp.lon) / 2,
        segIdx: idx + 1,
      }));
        ...
        try {
          ...
          if (res.status === 429) {
            results.push({ type:'apiError', severity:'warning',
              message:`Land check rate-limited after leg ${b} — enable SeaMarks overlay for visual hazards` });
            break;
          }
          ...
          (data.results || []).forEach((r, j) => {
            if (r.elevation !== null && r.elevation > 2) {
              ...
              results.push({ segIdx: seg.segIdx, type:'land', severity:'error',
                message:`Leg WP${seg.segIdx}→WP${seg.segIdx+1}: crosses land (elev. ${r.elevation.toFixed(0)}m at ${seg.lat.toFixed(3)}°N, ${seg.lon.toFixed(3)}°E)` });
            }
          });
        } catch (err) {
          results.push({ type:'apiError', severity:'warning',
            message:`Land check unavailable for legs ${b+1}–${b+BATCH}: ${err.message}` });
        }
        if (b + BATCH < midpoints.length) {
    ...
    // If no real issues found, push an OK result
    ...
    else if (warns  > 0) notify(`Route check: ${warns} warning${warns>1?'s':''} — review details`, 'success');
    else                 notify('Route check passed — no issues found ✅', 'success');
  };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, minHeight:0 }}>
      ...
      {/* ── Top bar (auto route name + export) ──────────────────────────── */}
          ...
          <div className="p-panel" style={{ overflowY:'auto' }}>
            ...
            {panel === 'auto' && (
              <>
                ...
                {showDbSugg && dbSuggestions.length > 0 && (
                  <div style={{ marginBottom:'1rem', background:'rgba(0,180,216,0.06)', border:'1px solid rgba(0,180,216,0.25)', borderRadius:10, padding:'10px' }}>
                    ...
                    <button className="btn btn-secondary" style={{ width:'100%', fontSize:'0.7rem', padding:'5px', marginTop:4 }}
                      onClick={() => { setShowDbSugg(false); fallbackAutoRoute(); }}>⚡ Skip — use Auto Route instead</button>
                  </div>
                )}
                ...
                {/* ← CHANGED: Ship Animation UI removed — state variables + MapView props kept */}
              </>
            )}

            {/* ══ MANUAL ROUTE BUILDER PANEL ════════════════════════════════ */}
            {panel === 'manual' && (
              <>
                {/* Header */}
                <div className="p-section">
                  <span className="p-label">✏️ Manual Route Builder</span>
                  <div style={{ fontSize:'0.72rem', color:'var(--text2)', marginBottom:'0.6rem', lineHeight:1.5 }}>
                    Click anywhere on the map to place waypoints. Drag to reposition. Course &amp; distance appear on each leg automatically.
                  </div>
                ...
                {/* Waypoint table with editable names */}
                {manualWps.length > 0 && (
                  <div className="p-section">
                    ...
                    <div style={{ overflowX:'auto' }}>
                      <table className="wp-table">
                        <thead>
                          <tr><th>#</th><th>Name</th><th>Lat</th><th>Lon</th><th>Crs°</th><th>NM</th><th>Del</th></tr>
                        </thead>
                        <tbody>
                          {manualWps.map((wp, i) => (
                            <tr key={i}>
                ...
                <div className="p-section">
                  ...
                  <div style={{ fontSize:'0.68rem', color:'var(--text2)', marginBottom:6, lineHeight:1.45 }}>
                    Checks: land crossings (GEBCO), piracy zones, ECA / SECA / MARPOL, anchorages, sharp turns, duplicate WPs.
                  </div>
                  ...
                  {checkResults.length > 0 && (
                    <div>
                      {checkResults.filter(r => r.type!=='ok').map((r, i) => (
                        <div key={i} style={{
                          padding:'6px 8px', marginBottom:4, borderRadius:6, fontSize:'0.7rem', lineHeight:1.4,
                ...
                <div className="p-section">
                  ...
                  <button className="btn btn-gold" style={{ width:'100%', justifyContent:'center' }}
                    disabled={manualWps.length<2} onClick={exportManualRoute}>
                    ⬇ Export Route
                  </button>
                </div>
                ...
                <div className="p-section">
                  ...
                  {savedRoutes.length === 0
                    ? <div style={{ fontSize:'0.72rem', color:'var(--text2)', textAlign:'center', padding:'1rem 0' }}>
                        No saved routes yet — build and save your first route!
                      </div>
                    : savedRoutes.map(r => (
