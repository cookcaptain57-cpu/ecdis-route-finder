...
const fetchPortsFromSheet = async () => {
  const tabs = ["Sheet1","Sheet2","Ports","World Ports","Port List","All Ports",
    "Data","Main","Asia","Europe","Americas","Africa","Middle East","India",
  ...
  for(const tab of tabs){
    try{
      const url = `https://docs.google.com/spreadsheets/d/${PORTS_SHEET_ID}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(tab)}`;
      const r = await fetch(url);
      if(!r.ok) continue;
      const csv = await r.text();
      const rows = parseCSV(csv);
      if(rows.length > 0){
        console.log(`Ports loaded from tab "${tab}": ${rows.length} rows`);
        return rows;
      }
    }catch(e){ continue; }
  }
